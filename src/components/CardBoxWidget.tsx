      ...
      {props.trendLabel && props.trendType && props.trendColor && (
        <div className="flex items-center justify-between mb-3">
          <PillTagTrend
            ...
            small
          />
          <BaseButton color="lightDark" small />
        </div>
      )}
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl leading-tight font-semibold">
            {/* <NumberDynamic value={8} prefix={props.numberPrefix} suffix={props.numberSuffix} /> */}
          </h1>
        </div>
        {props.icon && (
          <BaseIcon path={props.icon} size="48" h="h-16" className={colorsText[props.iconColor]} />
        )}
      </div>
    </CardBox>
  )
